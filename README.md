...
│                       → switch_root                             │
│                  └─ real root systemd:                          │
│                       var.mount + home.mount + srv.mount        │
│                         (explicit units, btrfs subvols —        │
│                          not gpt-auto)                          │
...
- OpenZFS upstream typically trails new kernels by 2-6 weeks. The build script uses an upstream-tarball flow (`mkosi.shared/zfs-build.sh`) and picks the version itself: the newest stable release whose META `Linux-Minimum`..`Linux-Maximum` covers the image kernel's major.minor. The build log prints it as `Building zfs sysext: zfs=<version> kver=<kernel>`. When it fails with `No stable OpenZFS release declares support for kernel`, upstream has not caught up yet. Recovery paths:
  - **Wait for OpenZFS** to publish a release covering the kernel, then re-run the build. No change needed.
  - **Pin `ZFS_VERSION`** (for example `ZFS_VERSION=2.4.4`) to skip the lookup and build a specific release, including one that works despite its META.
...
Nothing in the initrd phase reads `/var`, so the shipped `var.mount` unit mounts it post-pivot — same lifecycle as `home.mount` and `srv.mount`. All three are explicit units, **not** gpt-auto output: `systemd-gpt-auto-generator` is unreliable on verity-protected installs (systemd 259), so myosi owns every post-pivot mount and pins `Options=` itself (a gpt-auto `var.mount` would use btrfs defaults and lose `compress=zstd:3` + `noatime`). `/etc` is the special case — PID 1 reads `/etc/selinux/config` + `/etc/systemd/system.conf` synchronously at startup, so the mount must fire before pivot.
...
| **v1.5: OpenZFS sysext via upstream tarball** | ✅ done — `zfs-build.sh` pulls the newest `zfs-${ZFS_VERSION}.tar.gz` supporting the kernel, generates SRPMs with `make srpm-utils srpm-kmod`, rebuilds via `kmod_exec rpmbuild`, signs `zfs.ko` + `spl.ko`. No dependency on zfsonlinux.org/fedora packaging that lags new Fedora releases. |
